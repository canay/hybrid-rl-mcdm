...
from src.home_real_data import ITEMS_PATH, MANIFEST_PATH, PROC_DIR, USERS_PATH


OUTPUT_PATH = Path(__file__).resolve().parents[1] / "results" / "home_real_results.json"
...
def run_external_validation() -> dict:
    ...
    report = {
        "config": {
            "source_manifest": str(MANIFEST_PATH.relative_to(Path(__file__).resolve().parents[1])),
            "n_users": len(users),
            "n_items": n_items,
